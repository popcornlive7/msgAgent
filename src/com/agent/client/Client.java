...
import com.agent.msgserver.Server;
...
public class Client implements Runnable {
	...
	private void receive(Selector selector, SelectionKey key) {    
		try {      
    		System.out.println("client call receive!!");
			// 키 채널을 가져온다.      
			...
			// 데이터 끝이 개행 일 경우. + >      
			if (sb.length() > 3 
					&& sb.charAt(sb.length() - 3) == CR 
					&& sb.charAt(sb.length() - 2) == LF 
    ...
    private void send(Selector selector, SelectionKey key) {    
    	try {
    		System.out.println("client call send!!");
    		// 키 채널을 가져온다.      
    		SocketChannel channel = (SocketChannel) key.channel();
    		// 채널 Non-blocking 설정      
    		channel.configureBlocking(false);
    ...
    public static void main(String[] args) {    
    	// 포트는 10000을 Listen한다.    
    	Executors.newSingleThreadExecutor().execute(new Server("localhost", 9998));
        // 클라이언트 쓰레드    
    	Executors.newSingleThreadExecutor().execute(new Client("127.0.0.1", 9998));
    }
}
